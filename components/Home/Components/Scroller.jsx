      ...
      <div className="flex items-center gap-3 overflow-x-auto p-2 snap-x max-w-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
        {SectionNodes?.map((nav, idx) => (
          <a
            ...
            className="shrink-0 whitespace-nowrap px-6 py-2.5 rounded-full  backdrop-blur-md font-bold text-[13px] tracking-wide text-gray-600 hover:text-[#561b5e] bg-[#fff]  hover:bg-[#fff] hover:shadow-sm transition-all duration-300 snap-center"
          >
            {nav}
          </a>
        ))}
